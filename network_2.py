...
class Router:
    ...
    def process_network_packet(self, packet, i):
        ...
        # m_fr = pkt
        label = None
        if self.encap_tbl_D[i]:
            label = self.frwd_tbl_D[i][1]
            interface = self.frwd_tbl_D[i][3]
        m_fr = MPLSFrame(label, packet.to_byte_S())
        ...
        try:
            ...
            print('%s: forwarding frame "%s" from interface %d to %d' % (self, fr, i, interface))
        except queue.Full:
    ...
    def process_MPLS_frame(self, m_fr, i):
        ...
        # setting the defaul values before we assign them based on the table keys
        # they will be assigned based on forwarding table D or decapsulated table D
        # out_label = None
        # in_label = int(m_fr[0])
        # out_interface = None
        # out_link_label = None
        ...
        payload = m_fr  # payload is message
        ...
        # table_key = (in_label, i)
        # #first we check forwarding table keys
        # keys = self.frwd_tbl_D.keys()
        # table = self.frwd_tbl_D[i]
        # out = self.frwd_tbl_D[i][3]
        in_label = int(m_fr[:1]) # grabs the in label
        out_label = None
        out_interface = None
        fr = None
        for j in self.frwd_tbl_D:
            if self.frwd_tbl_D[j][0] == in_label: # checks that labels match
                out_interface = self.frwd_tbl_D[j][3] # sets the out interface
                break
        if not self.decap_tbl_D[i]:# enters no decapsulation needed
            ...
            out_link_label = 'MPLS'
            fr = LinkFrame(out_link_label, payload)  # makes packet an MPLS Frame
        # if table key isn't in forwarding table, check decapsulation table
            ...
            try:
                ...
                fr = LinkFrame(out_link_label, packet)
                self.intf_L[out_interface].put(fr.to_byte_S(), 'out', True)
                print('%s: forwarding frame "%s" from interface %d to %d' % (self, fr, i, out_interface))
            except queue.Full:
                print('%s: frame "%s" lost on interface %d' % (self, p, i))
