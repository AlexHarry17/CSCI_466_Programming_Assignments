...
class Router:
    ...
    def update_routes(self, p, i):
        ...
        packet_table = p.data_S.split("///")
        print("Packet_table", packet_table)

        #split by // to get routes in table
        table_routes = packet_table[1].split("//")
        length = len(table_routes)
        table_routes = table_routes[0:length-1] #remove the last route, as it is empty
        print("Table_routes", table_routes)

        #For each route in the table
        for table_route in table_routes:
            #split route into it's components "destination/sending router(link)/cost"
    ...
    def print_routes(self):
        ...
        for key in self.rt_tbl_D[self.name].keys():
            ...
            for _, v in self.rt_tbl_D.items():
                if key in v:
                    val = v[key]
                    if val == 1000: #will set to 1000 if not available to update, no link known
                        val = 'X'
                body += str(val) + "  |   "
        print(body)
        print(border)
